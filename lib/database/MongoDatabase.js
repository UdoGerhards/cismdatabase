...
class MongoDatabase extends Base {
  ...
  */
  ...
  async spec_getQuestionFullRandom(count) {
    ...
    return await instance.questionDAO.collection
      .aggregate([
        { $sample: { size: count } }, // Bleibt wie gewünscht erhalten
        {
          $lookup: {
            from: "answer", // Die Ziel-Collection
            localField: "_id", // Das Feld aus "question" (ObjectId)
            foreignField: "question_id", // Das Feld aus "answer" (ObjectId)
            as: "answers", // Der Name des neuen Arrays im Ergebnis
          },
        },
      ])
      .toArray();

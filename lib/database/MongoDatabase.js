    ...
    const aggregateObj = [
      {
        // 🎯 1. Test auswählen
        $match: {
          _id: { $eq: { $toObjectId: testId } },
        },
      },
      {
        // 🔗 2. test_answers joinen
        $lookup: {
          from: "test_answer",
          let: { testIdStr: { $toString: "$_id" } },
          pipeline: [
            ...
            // 🔗 3. question joinen
            {
              $lookup: {
                ...
                pipeline: [
                  ...
                  // 🔗 4. korrekte Antwort holen
                  {
                    $lookup: {
                      from: "answer",
                      let: {
                        qIdStr: { $toString: "$_id" },
                        correctKey: "$correct", // z.B. "D"
                      },
                      pipeline: [
                        {
                          $match: {
                  ...
                  {
                    // 📦 CorrectAnswer als Objekt statt Array
                    $addFields: {
                      CorrectAnswer: { $arrayElemAt: ["$CorrectAnswer", 0] },
                    },
                  },
            ...
            {
              // 📦 question als Objekt statt Array
              $addFields: {

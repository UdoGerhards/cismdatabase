...
class MongoDatabase extends Base {
  ...
  */
  ...
  async init(dbString, dbName) {
    ...
    this.testAnswers = new MongoCollectionReadWrite(
      test_answers,
      testAnswerValidator,
    );
  ...
  async spec_getTestResult(start, end) {
    ...
    return await this.testMeta.aggregate(aggregateObj);
  }
  ...
  async spec_getQuestionFull(id) {
    ...
    const result = await this.questionCollection.aggregate(aggregateObj);

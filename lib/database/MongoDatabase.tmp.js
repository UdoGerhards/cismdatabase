...
class MongoDatabase extends Base {
  ...
  async init(logManager) {
    ...
    const userValidator = (user) => {
      ...
      // -----------------------------------------
      // 3. twoFactor (optional, verschachtelt)
      // -----------------------------------------
      if (user.twoFactor !== undefined) {
        if (typeof user.twoFactor !== "object") {
          throw new Error("twoFactor must be an object");
        }

        if (
          user.twoFactor.enabled !== undefined &&
          typeof user.twoFactor.enabled !== "boolean"
        ) {
          throw new Error("twoFactor.enabled must be boolean");
        }
        ...
        if (
          ...
          throw new Error("twoFactor.secret must be string or null");
        }

        if (
          user.twoFactor.tempSecret !== undefined &&
          user.twoFactor.tempSecret !== null &&
          typeof user.twoFactor.tempSecret !== "string"
        ) {
          throw new Error("twoFactor.tempSecret must be string or null");
        }
      }
  ...
  async getUser(email) {
    ...
    return this.normalizeUser(user); // FIX
  }
  ...
  async changePassword(userId, newPassword) {
    ...
    try {
      ...
      // 2. Update
      // ----------------------------------
      const success = await instance.updateUser(userId, {
        passwordHash: passwordHash, // FIX
  ...
  async createUser({ email, firstname, lastname }) {
    ...
    try {
      instance.logger.debug("Creating user: " + email);

      // ----------------------------------
      // 1. prüfen ob User existiert
      // ----------------------------------
      const existing = await instance.userDAO.findOne({ email: email.trim() });
      ...
      const user = {
        ...
        // 👉 Pflicht für dein DAO
        init() {},
      };

      // ----------------------------------
      // 5. Speichern über DAO
      // ----------------------------------
      const createdUser = await instance.userDAO.create(user);

      instance.logger.debug("User created: " + createdUser?._id);
  ...
  */
  async updateUser(id, updateObject) {
    ...
    try {
      instance.logger.debug("Updating user by ID: " + id);
      instance.logger.debug(JSON.stringify(updateObject, null, 2));

      const result = await instance.userDAO.update(id, updateObject);

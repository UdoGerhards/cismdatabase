...
class MongoCollectionReadOnly extends Base {
  ...
  findOne = async (search) => {
    let instance = this;
    try {
      instance.logger.debug("Searching only one entry");
      instance.logger.debug(JSON.stringify(search, null, 2));

      const doc = await instance.collection.findOne(search);

      instance.logger.debug(`Found: ${doc}`);
  ...
  async getTotalCount() {
    const instance = this;
    return await instance.collection.countDocuments({});
  }
  ...
  aggregate = async (pipeline) => {
    ...
    try {
      ...
      instance.logger.debug("Aggregation result: ");
      instance.logger.debug(JSON.stringify(docs, null, 2));

      const result = docs.map((d) => ({
        // Nur wenn d._id existiert, wird .toString() aufgerufen
        // Falls nicht, bleibt 'id' undefined oder wird gar nicht erst gesetzt
        ...(d._id ? { id: d._id.toString() } : {}),
        ...d,
      }));

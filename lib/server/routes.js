  ...
  registerRoutes() {
    ...
    */
    ...
    instance.app.post("/api/user", async (req, res) => {
      ...
      if (!email) {
        return res.status(401).json({ error: "Email not found in certificate" });
      }
      ...
      try {
        ...
        instance.logger.info(`User authenticated! ID: ${user._id}, Firstname: ${user.firstname}, Lastname: ${user.lastname}, E-Mail: ${user.email}`);
    ...
    */
    ...
    instance.app.get("/api/auth", instance.authenticateJWT.bind(instance), (req, res) => {
      res.json({
        authenticated: true,
        user: req.user,
      });
    ...
    instance.app.post(
      ...
      async (req, res) => {
        instance.logger.info(
          `/api/question requested by ${req.user.email}`
        );
